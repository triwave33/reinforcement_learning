...
for epi in range(num_episode): 
    ...
    while(done == False):
        ...
        value_est = critic.estimate_value_from_state(state) 
        action, log_prob = policy.select_action(state)
        a_list.append(action)
        #print("action: %d" % action)
        state, reward, done, info, _ = env.step(action)
        step_count +=1
        v_list.append(value_est)
    ...
    if use_for_loop:
        for i,(s,a,G, log_prob,v) in enumerate(zip(s_list, a_list, G_list, policy.log_probs_list, v_list)):
           
            delta = (G-v).detach()
            delta_v_list.append((-v * delta).reshape(-1,1))
            delta_lnPi_list.append((-log_prob * delta ).reshape(-1,1))
        delta_v_all = torch.cat(delta_v_list).sum()
        delta_lnPi_all = torch.cat(delta_lnPi_list).sum()
    else:
        G_tensor = torch.tensor(G_list).unsqueeze(1)
        v_tensor = torch.cat(v_list)
        delta_tensor = (G_tensor - v_tensor).detach()
        delta_v_all = (-v_tensor * delta_tensor).sum()
        lnPi_tensor = torch.cat(policy.log_probs_list).unsqueeze(1)
        delta_lnPi_all = (- lnPi_tensor * delta_tensor).sum()

    # update critic
    critic_optimizer.zero_grad()
    delta_v_all.backward(retain_graph=True)
    critic_optimizer.step()
    # update policy 
    policy_optimizer.zero_grad()
    delta_lnPi_all.backward()
    policy_optimizer.step()
...
np.save("REINFORCE_base_line.npy", np.array(total_reward_list))
